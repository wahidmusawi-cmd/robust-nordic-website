...
async function adminMiddleware(req: NextRequest): Promise<NextResponse> {
  ...
  if (pathname === "/admin/kirjaudu") return NextResponse.next()

  const secret = getAdminSecret()
  if (!secret) {
    // No password configured: allow in development, force the setup screen in production.
    if (process.env.NODE_ENV !== "production") return NextResponse.next()
    return NextResponse.redirect(new URL("/admin/kirjaudu", req.url))
  }
  ...
  const loginUrl = new URL("/admin/kirjaudu", req.url)
  // Preserve the query string so re-login lands back on the same filters/page.
  const target = pathname + req.nextUrl.search
  if (target !== "/admin") loginUrl.searchParams.set("seuraava", target)
